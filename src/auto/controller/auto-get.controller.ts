...
import { Auto, AutoArt } from '../entity/auto.entity.js';
...
export class AutoQuery implements Suchkriterien {
    ...
    @ApiProperty({ required: false })
    declare readonly baujahr?: number;
    ...
    @ApiProperty({ required: false })
    declare readonly esb?: string;

    @ApiProperty({ required: false })
    declare readonly abs?: string;

    @ApiProperty({ required: false })
    declare readonly airbag?: string;

    @ApiProperty({ required: false })
    declare readonly parkassistent?: string;

    @ApiProperty({ required: false })
    declare readonly motor?: string;
...
export class AutoGetController {
    ...
    async getById(
        ...
        if (this.#logger.isLevelEnabled('debug')) {
            ...
            this.#logger.debug('getById(): motor=%o', auto.motor);
        }
    ...
    async get(
        ...
        const autoSlice = await this.#service.find(query, pageable);
        const autoPage = createPage(autoSlice, pageable);
